...
Cloud instance of robot city is used for tasks that require constant uptime, and would be insecure or risky to run on your local computer, along with hosting your data and admin dashboard UI. Local instance of robotcity is used for workflows that require residential IP. Seamless switching between cloud and local instance. All conversations start in the cloud and certain tool calls or packaged workflows can be requested to run on the local machine.

First party "jobs" can be selected for your assistant to schedule and execute each with their own set of tools. Primitives for AI assisted development and testing for new "jobs" and versioning for them.
...
When you are planning out changes with a harnesses "plan mode", make sure to write out your finalized plan ONCE APPROVED to docs/ directory. The .md file name can be long and descriptive I should be able to immedietly tell what the file was used for by the title of it. Additionally update the SPEC.md key decision log with a sentence.
