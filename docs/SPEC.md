...
- **Drizzle ORM over bun:sqlite for typed queries.** `src/db/client.ts` exports the Drizzle `db` (all feature queries) and the raw `sqlite` handle (boot-time DDL in `schema.ts`, PRAGMAs, FTS5 `MATCH` queries — things an ORM can't model). Drizzle table definitions live in `src/db/tables.ts` and mirror the raw DDL; migrations stay as the idempotent boot-time `migrate()`, no drizzle-kit. Chosen over Prisma/TypeORM because it needs no codegen/build step and runs natively on Bun.
...
- **Assistant benchmarks are ad hoc live-model evaluations.** Calendar-v1 runs the real Discord tool loop against fake Calendar/Discord state, scores intent/action/outcome/reply across three trials per scenario, and publishes only explicit result snapshots for named stage-model configurations. It is not part of CI or the production pipeline.
